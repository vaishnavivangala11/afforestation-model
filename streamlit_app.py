...
co2_rate = selected_tree["CO2_per_year_kg"]
survival_rate = selected_tree["Survival_rate"]
growth_factor = selected_tree["Growth_factor"]
soil_type = selected_tree["Soil_Type"]

# Adjusted CO₂ calculation
adjusted_co2 = age * co2_rate * survival_rate * growth_factor

# Display result
st.success(f"🌱 A {tree} tree absorbs approx. **{adjusted_co2:.1f} kg of CO₂** over {age} years (adjusted for growth & survival).")
st.info(f"📍 Soil Type Recommended for {tree}: **{soil_type}**")

# CO₂ Sequestration Over 20 Years (for graph)
st.subheader("📈 CO₂ Sequestration Over 20 Years")

years = list(range(1, 21))
co2_values = [co2_rate * y * survival_rate * growth_factor for y in years]

fig, ax = plt.subplots()
ax.plot(years, co2_values, marker='o', color='green')
ax.set_xlabel("Year")
ax.set_ylabel("Cumulative CO₂ Captured (kg)")
ax.set_title(f"CO₂ Capture by {tree} Over 20 Years")
st.pyplot(fig)

# What if 1000 Trees Are Planted?
st.subheader("🧠 What if 1000 Trees Are Planted?")

co2_1000_trees = [value * 1000 for value in co2_values]
...
ax2.set_title(f"CO₂ Sequestration for 1000 {tree} Trees Over 20 Years")
st.pyplot(fig2)

st.success(f"🌍 Planting 1000 {tree} trees can absorb **{total_20_years:,.0f} kg** of CO₂ in 20 years.")
...
if st.button("📄 Create and Download PDF Report"):
    ...
    pdf.ln(10)
    pdf.cell(200, 10, txt=f"Tree Species: {tree}", ln=True)
    pdf.cell(200, 10, txt=f"Tree Age: {age} years", ln=True)
    pdf.cell(200, 10, txt=f"Soil Type: {soil_type}", ln=True)
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as tmpfile:
        ...
        ax3.set_title(f"1000 {tree} Trees Over 20 Years")
